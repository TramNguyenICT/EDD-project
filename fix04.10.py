...
def airport_quiz(airport_id):
    ...
    if airport_id == 1059:
        ...
        if answer1059 == "map" or answer1059 == "themap" or answer1059 == "onthemap" or answer1059 == "inthemap":
            print("Brilliant! You are so smart! You got 10 more letters!")
            letter_count += letter_change
        else:
            print("It's the map!")
...
print(rova_script)
#cap nhat current airport = 1060

#lay letter count tron database
sql99 =f"SELECT letter_count FROM player WHERE player_name = '{player_name}'"
cursor.execute(sql99)
final_letter_count_tuple = cursor.fetchall()
final_letter_count = final_letter_count_tuple[0][0]
